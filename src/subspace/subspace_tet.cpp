...
profiler.Start("OMP Assemble");
  ...
  for (int j = 0; j < int(v_list_1_.size()); ++j) {
     ...
     // }
     // else {
      //   result_pairs_1[j] = old_vert_basis_transpose_[v] * (sub_k) * old_vert_basis_[v];
      //}
  }
  ...
  for (int i = 0; i < int(vv_list_1_.size()); ++i) {
    ...
    //}else {
    //    result_pairs_1[offset_1+i] = old_vert_basis_transpose_[vi]* sub_k_1 * old_vert_basis_[vj];
    //    result_pairs_1[offset_1+i] += old_vert_basis_transpose_[vj]* sub_k_2 * old_vert_basis_[vi];
    //}
  }
...
/*for(int i = 0; i <result_pairs_1.size();i++) {
  ...
  for (int i=0;i<cubature_flip_counter_;i++) {
    ...
    for (int local_v = 0; local_v < 4; ++local_v) {
      ...
      //if(sim_mode_!=2) {
       subspace_force_1 -= vert_basis_transpose_[v] * (weight * map_force);
     // } else {
      //   subspace_force_1 -= old_vert_basis_transpose_[v] * (weight * map_force);
     // }
      //subspace_force_1 -= vert_basis_transpose_[v] * (weight * map_force);
    }
  }
  ...
  for (int j = 0; j < int(v_list_2_.size()); ++j) {
      ...
      //}
      //else {
       //  result_pairs_2[j] = old_vert_basis_transpose_[v] * (sub_k) * old_vert_basis_[v];
      //}
  ...
  for (int i = 0; i < int(vv_list_2_.size()); ++i) {
    ...
    //}else {
    //    result_pairs_2[offset_2+i] = old_vert_basis_transpose_[vi]* sub_k_1 * old_vert_basis_[vj];
    //    result_pairs_2[offset_2+i] += old_vert_basis_transpose_[vj]* sub_k_2 * old_vert_basis_[vi];
   // }
    //result_pairs_2[offset_2+i] = vert_basis_transpose_[vi]* sub_k_1 * vert_basis_[vj];
    //result_pairs_2[offset_2+i] += vert_basis_transpose_[vj]* sub_k_2 * vert_basis_[vi];
  }
...
/*for(int i = 0; i <result_pairs_2.size();i++) {
  ...
  for (int i=cubature_flip_counter_;i<cubature_.size();i++) {
    ...
    for (int local_v = 0; local_v < 4; ++local_v) {
     ...
     // if(sim_mode_!=2) {
       subspace_force_2 -= vert_basis_transpose_[v] * (weight * map_force);
      //} else {
      //   subspace_force_2 -= old_vert_basis_transpose_[v] * (weight * map_force);
      //}
    }
  }
  ...
  /*if(sim_mode_ == 2 && hasPreCompPDK_)
  {
      subspace_force = precompPdk_ * subspace_force;
      Mat temp = precompPdk_ * reduced_k;
      reduced_k = temp * precompPdk2_;
  }*/
  ...
  if(sim_mode_ == 2) {
      ...
      subspace_force -= my_f_reduced;
       ...
       //Vec3 f(-1158.0,-163.0,805.0);
        Vec3 f(-2316.0,-326.0,1610.0);
       //subspace_force += vert_basis_transpose_[13084] * f * 1.0 ;
  }
  ...
  if (ui_force_vert >= 0) {
      ...
      //} else {
      //   subspace_force += old_vert_basis_transpose_[ui_force_vert] * ui_force *1.0 ;
      //}
  }
  ...
  memcpy(render_X,X,sizeof(double)*vertex_num_ * 3);
   double *tempStorage = new double[vertex_num_ * 3];
 ...
 if(global::sim_state==3) {
    ...
    /*memcpy(tempStorage,render_X,sizeof(double)*vertex_num_ * 3);



     OMP_FOR
     for(int i=0;i < int(level_1_verts_.size()); i++) {
         double x=0.0,y=0.0,z=0.0;
         int ne = level_1_neighbours_[i].size();
         for(int ni:level_1_neighbours_[i]) {
             x += (1.0)*render_X[3*ni+0];
             y += (1.0)*render_X[3*ni+1];
             z += (1.0)*render_X[3*ni+2];
         }
         tempStorage[3*level_1_verts_[i]+0] = x/ne;
         tempStorage[3*level_1_verts_[i]+1] = y/ne;
         tempStorage[3*level_1_verts_[i]+2] = z/ne;
     }

     memcpy(render_X,tempStorage,sizeof(double)*vertex_num_ * 3);
...
void SubspaceTet::OnSubspaceLoaded2() {
  ...
  //precompPdk_ = basis_transpose_ * old_basis_;
  //precompPdk2_ = old_basis_transpose_ * basis_;
  //hasPreCompPDK_ = true;
...
void SubspaceTet::Simulate(double dt) {
  ...
  if (ui_force_vert >= 0) {
    P(dj::Vec3d(ui_force));
    full_force[ui_force_vert * 3 + 0] += 1.0*ui_force[0];
    full_force[ui_force_vert * 3 + 1] += 1.0*ui_force[1];
    full_force[ui_force_vert * 3 + 2] += 1.0*ui_force[2];
  }
