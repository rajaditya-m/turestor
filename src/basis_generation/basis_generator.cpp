...
BasisGenerator::BasisGenerator(int vert_num, double* vert_pos,
	...
	double density,
	double* mass,
	std::set<int> *fixed_vertex_set, bool isConstrained) {
...
BasisGenerator::BasisGenerator(TetMesh* mesh, double *mass, std::set<int> *fixed_vertex_set, bool isConstrained) {
...
void BasisGenerator::ComputeModalDerivatives() {
	...
	int rigidCombinations = rigid_mode_num_*(rigid_mode_num_ + 1) / 2;
	int nonRigidCombinations = numUsedLinearModes * (numUsedLinearModes + 1) / 2;

	P(rigidCombinations);
	P(nonRigidCombinations);

	modal_derivative_num_ = rigidCombinations + nonRigidCombinations;
	...
	OMP_FOR
		for (int k = 0; k < rigidCombinations; k++) {
		int n = rigid_mode_num_;
		int i = n - floor(sqrt(-8 * k + 4 * n*(n + 1) - 7) / 2.0 - 0.5) - 1;
		int j = k + i - n*(n + 1) / 2 + (n - i)*((n - i) + 1) / 2;
		stVKStiffnessHessian->EvaluateHessianQuadraticForm(
			&Ulin[n3 * i], &Ulin[n3 * j], &rhs[ELT(n3, 0, k)]);
		//P(i,j,k);
		}

	OMP_FOR
		for (int k = 0; k < nonRigidCombinations; k++) {
		int n = numUsedLinearModes;
		int i = n - floor(sqrt(-8 * k + 4 * n*(n + 1) - 7) / 2.0 - 0.5) - 1;
		int j = k + i - n*(n + 1) / 2 + (n - i)*((n - i) + 1) / 2;
		stVKStiffnessHessian->EvaluateHessianQuadraticForm(
			&Ulin[n3 * (rigid_mode_num_ + i)], &Ulin[n3 * (rigid_mode_num_ + j)], &rhs[ELT(n3, 0, k + rigidCombinations)]);
		//P(i,j,k);
		}

	OMP_FOR
		for (int k = 0; k < (n3 * modal_derivative_num_); k++) //multiply by -1
			rhs[k] *= -1.0;
...
int j = k + i - n*(n + 1) / 2 + (n - i)*((n - i) + 1) / 2;
stVKStiffnessHessian->EvaluateHessianQuadraticForm(
&Ulin[n3 * (rigid_mode_num_ + i)], &Ulin[n3 * (rigid_mode_num_ + j)], &rhs[ELT(n3, 0, k)]);
//P(i,j,k);
}
...
int j = k + i - n*(n + 1) / 2 + (n - i)*((n - i) + 1) / 2;
stVKStiffnessHessian->EvaluateHessianQuadraticForm(
&Ulin[n3 * (rigid_mode_num_ + i + 6)], &Ulin[n3 * (rigid_mode_num_ + j + 6)], &rhs[ELT(n3, 0, k + 21)]);
//P(i+6,j+6,k+21);
}
...
bool BasisGenerator::MassPCA(int input_basis_num, int output_basis_num, std::vector<double>& basis, int data_origin) {
	...
	if (data_origin == kModalAnalysis) {
		...
		int nonRigidCombinations = numUsedLinearModes * (numUsedLinearModes + 1) / 2;
		int rigidCombinations = rigid_mode_num_ * (rigid_mode_num_ + 1) / 2;
		numDataVectors = rigidCombinations + nonRigidCombinations + linear_mode_num_;
		P(rigidCombinations, nonRigidCombinations, linear_mode_num_);
		...
		if (rigid_mode_num_)
		{
			...
			for (int i = 0; i < linear_mode_num_; i++) {
				...
				if (i < rigid_mode_num_) {
					factor = 1.0;// (max)-(i*diff);
				}
				else {
					lambda = frequencies_[i] * frequencies_[i];
					factor = lambda0 / lambda;
				}
				for (int vertex = 0; vertex < n3; vertex++)
					dataMatrix[ELT(n3, vertex, i)] = factor * Ulin[ELT(n3, vertex, i)];
			}
			...
			max = 0.0008;
			min = 0.000008;
			diff = 1.0;// (max - min) / (rigidCombinations - 1);
			int ctr = 0;
			for (int i = 0; i < rigid_mode_num_; i++) {
				for (int j = i; j < rigid_mode_num_; j++) {
					double factor = 1.0;// (max)-(ctr*diff);
					//std::cout << "MD i:" << i << " j:" << j << " Factor:" << factor << "\n";
					for (int vertex = 0; vertex < n3; vertex++)
						dataMatrix[ELT(n3, vertex, linear_mode_num_ + pos)] =
						factor * normalizedModalDerivatives[ELT(n3, vertex, pos)];
					pos++;
					ctr++;
				}
			}
			for (int i = rigid_mode_num_; i < numUsedLinearModes; i++) {
				for (int j = i; j < numUsedLinearModes; j++) {
...
void BasisGenerator::RegenerateAllModes(int modeId){
	...
	P(stiffyMultiplier_);
	double stiffness = 5.0e6;// * (double)stiffyMultiplier_;
