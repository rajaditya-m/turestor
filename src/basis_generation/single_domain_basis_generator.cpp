...
SingleDomainBasisGenerator::SingleDomainBasisGenerator(const char *mesh_file, AffineTransformer<double> *transformer, bool loadInterface)
  : SubspaceTet(mesh_file, 0, transformer,true, loadInterface) {
  fixed_verts_.clear();
}
...
void SingleDomainBasisGenerator::GenerateBasis(const char *basis_prefix, int linear_basis_num, int final_basis_num, bool writeStuff) {
  basis_generator = new BasisGenerator(inv_fem_->vega_mesh_, &mass_[0]);
