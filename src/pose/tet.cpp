...
void Tet::Save_OBJ_File(const char * name) {
  ...
  for (int i = 0; i < vertex_num_; i++)
    fprintf(fp, "v %f %f %f\n", render_X[i * 3 + 0], render_X[i * 3 + 1], render_X[i * 3 + 2]);
  for (int i = 0; i < triangle_num_; i++)
...
void Tet::Build_Boundary_Triangles() {
  ...
  for(auto it= interface_vertices_.begin(); it!= interface_vertices_.end();it++) {
      ...
      incident_triangle_renderer__2[j] = newV;
  ...
  //Populate the laplacian data structures
  std::set<int> next_level_;
 for(auto it= interface_vertices_.begin(); it!= interface_vertices_.end();it++) {
      int j = it->second;
      int i = it->first;
      ...
      std::set<int> neighbours_i;
      for(auto it = incidence_vector_i.begin();it != incidence_vector_i.end();it++ ) {
          int t = *it;
          if(T[t*3+0] != j && T[t*3+0]!= i)
          {
              neighbours_i.insert(T[t*3+0]);
              next_level_.insert(T[t*3+0]);
          }
          if(T[t*3+1] != j && T[t*3+1]!= i)
          {
              neighbours_i.insert(T[t*3+1]);
              next_level_.insert(T[t*3+1]);
          }
          if(T[t*3+2] != j && T[t*3+2]!= i)
          {
              neighbours_i.insert(T[t*3+2]);
              next_level_.insert(T[t*3+2]);
          }
      }

      std::vector<int> i_n(neighbours_i.begin(),neighbours_i.end());

      std::set<int> neighbours_j;
      for(auto it = incidence_vector_j.begin();it != incidence_vector_j.end();it++ ) {
          int t = *it;
          if(T[t*3+0] != j && T[t*3+0]!= i)
          {
              neighbours_j.insert(T[t*3+0]);
              next_level_.insert(T[t*3+0]);
          }
          if(T[t*3+1] != j && T[t*3+1]!= i)
          {
              neighbours_j.insert(T[t*3+1]);
              next_level_.insert(T[t*3+1]);
          }
          if(T[t*3+2] != j && T[t*3+2]!= i)
          {
              neighbours_j.insert(T[t*3+2]);
              next_level_.insert(T[t*3+2]);
          }
      }

      std::vector<int> j_n(neighbours_j.begin(),neighbours_j.end());

      level_1_verts_.push_back(j);
      level_1_neighbours_.push_back(j_n);
      level_1_verts_.push_back(i);
      level_1_neighbours_.push_back(i_n);
  ...
  /*for(auto it = next_level_.begin(); it != next_level_.end(); it++) {
      ...
      auto t_it = std::find(level_1_verts_.begin(),level_1_verts_.end(),j);
      if(t_it!=level_1_verts_.end())
          continue;
